# This file estimates parameters using the NPL estimator in AM07
...
param iniP { i in I, (xs,xa) in X, a in A} = 
if a==1 then P_start[i, xs, xa] else (1-P_start[i, xs, xa]) ;

param P_kminus1 { i in I, (xs,xa) in X, a in A} ;

# Variables
var V {I,X} ;     #value functions
var P {I,X,A} >=0, <=1;

param fP {(xsnext,xanext) in X, (xs,xa) in X} = S_trans[xsnext,xs]*(prod {i in I} P_kminus1[i,xs,xa,if i in Apow[xanext] then 1 else 0]);

param fPchoice {i in I, (xsnext,xanext) in X, (xs,xa) in X, a in A} = if (if i in Apow[xanext] then 1 else 0)==a then fP[xsnext,xanext,xs,xa]/P_kminus1[i,xs,xa,if i in Apow[xanext] then 1 else 0];

param e {i in I,(xs,xa) in X,a in A} = euler_const - log(P_kminus1[i,xs,xa,a]); #assuming sigma=1
...
var epayoff {i in I, (xs,xa) in X, a in A} = sum {aanext in Aset:(if i in Apow[aanext] then 1 else 0)==a} ((prod {j in I:i<>j} P_kminus1[j,xs,xa,if j in Apow[aanext] then 1 else 0])*payoffpp[i,xs,aanext,if i in Apow[xa] then 1 else 0]);
...
maximize NPLobj: 
 	...
 	sum{i in I, m in M, t in 2..NT, (xs,xa) in X, a in A: ObsXS[m,t]==xs and ObsXA[m,t]==xa and ObsA[i,m,t]==a} log(P[i,xs,xa,a]);

subject to

Bellman {i in I, (xs,xa) in X}:
V[i,xs,xa]= sum {a in A} P_kminus1[i,xs,xa,a]*(epayoff[i,xs,xa,a] + e[i,xs,xa,a]) + beta*(sum {(xsnext,xanext) in X} V[i,xsnext,xanext]*fP[xsnext,xanext,xs,xa]);
